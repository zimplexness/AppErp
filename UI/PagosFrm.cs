    ...
    public partial class PagosFrm : MetroFramework.Forms.MetroForm
    {
        ...
        private void metroButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<FacturasPendientesPago_Result> FacturasPendientesPago = ComprobanteController.GetAllFacturasPedientes(IDproveedor).ToList();
                metroGridComprobantes.AutoGenerateColumns = false;
                metroGridComprobantes.DataSource = FacturasPendientesPago;
                metroGridComprobantes.AutoSize = false;

                foreach (var item in FacturasPendientesPago)
                {
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (DataGridViewRow row1 in metroGridComprobantes.Rows)
                    {
                        ...
                        if (Convert.ToBoolean(ck.Value) == true)
                        {
                            TotalImporteComprobante += Math.Round(Convert.ToDouble(row1.Cells["column5"].Value.ToString()), 2);
                        }
                    }
                    ...
                    foreach (DataGridViewRow rown1 in metroGridMedioPAgo.Rows)
                    {
                        TotalMediosPago += Convert.ToDouble(rown1.Cells["ImportePago"].Value.ToString().Replace(".", ","));
                            ...
                            foreach (DataGridViewRow row2 in metroGridComprobantes.Rows)
                            {
                                ...
                                {
                                    Detalle.idComprobante = int.Parse(row2.Cells["column2"].Value.ToString());
                            ...
                            if (dialogResult == DialogResult.Yes)
                            {
                                ...
                                foreach (DataGridViewRow row2 in metroGridComprobantes.Rows)
                                {
                                    ...
                                    if (Convert.ToBoolean(ck3.Value) == true)
                                    {
                                        ...
                                        ComprobanteController.ActualizarEstadoComprobante(int.Parse(row2.Cells["column2"].Value.ToString()));
        ...
        private void metroButton6_Click(object sender, EventArgs e)
        {
            ...
            List<SelectPagosProveedores_Result> Pagos = PagosController.GetPagosProveedores(metroDateTimeInicio.Value,metroDateTimeFin.Value).ToList();
            metroGridPagosProveedores.AutoGenerateColumns = false;
            metroGridPagosProveedores.DataSource = Pagos;
            metroGridPagosProveedores.AutoSize = false;
            ...
            foreach (var item in Pagos)
            {
                ...
                NumeroOrden.DataPropertyName = "NumeroOrdenPago";
        ...
        private void metroButton11_Click(object sender, EventArgs e)
        {
            //Declaro la instancia del ReporteViwer y lo llamo
            ReportesForm reporte = new ReportesForm();
            reporte.idpagoprop = int.Parse(metroGridPagosProveedores.CurrentRow.Cells["NumeroOrden"].Value.ToString());
            reporte.ShowDialog();
            
            //Declaro la instancia del ReporteViwer y lo llamo
            ReportRetencionesForm ReporRetencion = new ReportRetencionesForm();
            ReporRetencion.Idpago = int.Parse(metroGridPagosProveedores.CurrentRow.Cells["NumeroOrden"].Value.ToString());
